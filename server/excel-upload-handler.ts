...
export async function processExcelUpload(
  ...
  try {
    ...
    if (workbook.SheetNames.length === 0) {
      return res
        .status(400)
        .json({ message: "Excel file does not contain any sheets" });
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const nationality = row[headerIndices["Nationality"]]
          ?.toString()
          .trim();
        const yearsOfExperience = row[headerIndices["Years of Experience"]]
          ?.toString()
          .trim();
        const asset = row[headerIndices["Asset"]]?.toString().trim();
        const roleCategory = row[headerIndices["Role Category"]]
          ?.toString()
          .trim();
        const seniority = row[headerIndices["Seniority"]]?.toString().trim();
        const organizationName = row[headerIndices["Organization Name"]]
          ?.toString()
          .trim();
        ...
        let subCategoryIndex = headers.findIndex((header) => {
          ...
          const cleanHeader = header.toLowerCase().trim().replace(/\*/g, "");
          return (
            cleanHeader === "sub-category" || cleanHeader === "subcategory"
          );
        });
        ...
        else if (!validExperience.includes(yearsOfExperience))
          validationErrors.push(
            `Invalid experience. Must be one of: ${validExperience.join(", ")}`
          );
        if (!asset) validationErrors.push("Asset is required");
        else if (!validAssets.includes(asset))
          validationErrors.push(
            `Invalid asset. Must be one of: ${validAssets.join(", ")}`
          );
        if (!roleCategory) validationErrors.push("Role Category is required");
        else if (!validRoleCategories.includes(roleCategory))
          validationErrors.push(
            `Invalid role category. Must be one of: ${validRoleCategories.join(
              ", "
            )}`
          );
        if (!seniority) validationErrors.push("Seniority is required");
        else if (!validSeniority.includes(seniority))
          validationErrors.push(
        ...
        console.log(
          `Successfully created user: ${firstName} ${lastName} (${email})`
        );
      } catch (error) {
