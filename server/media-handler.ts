...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
      ...
      cb(new Error(`File type ${file.mimetype} not allowed. Supported types: images, PDFs, videos`));
    }
  }
});
...
export async function bulkDeleteMediaFiles(req: Request, res: Response) {
  try {
    ...
    // Delete from database
    const deletedCount = await storage.deleteMultipleMediaFiles(ids.map(id => parseInt(id)));
