...
export function useBlockAssessmentProgress(userId: number, courseId: number, unitId: number) {
  const queryClient = useQueryClient();

  // Fetch block progress for a specific unit in a course
  const { data: blockProgress = [], isLoading: isLoadingBlocks, error: blockError } = useQuery<UserBlockProgress[]>({
    queryKey: [`/api/progress/block/${courseId}/${unitId}`],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/progress/block/${courseId}/${unitId}`);
      return res.json();
    },
  ...
  const { data: assessmentProgress = [], isLoading: isLoadingAssessments, error: assessmentError } = useQuery<UserAssessmentProgress[]>({
    queryKey: [`/api/progress/assessment/${courseId}/${unitId}`],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/progress/assessment/${courseId}/${unitId}`);
      return res.json();
    },
    enabled: !!userId && !!courseId && !!unitId,
  });
  ...
  const markBlockCompleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/progress/block/${courseId}/${unitId}`] });
    },
  });
  ...
  const markAssessmentCompleteMutation = useMutation({
    mutationFn: async ({ assessmentId }: { assessmentId: number }) => {
      const res = await apiRequest("POST", "/api/progress/assessment/complete", {
        courseId,
        unitId,
        assessmentId,
      });
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/progress/assessment/${courseId}/${unitId}`] });
    },
  });

  // Helper functions to check completion status
  const isBlockCompleted = (blockId: number): boolean => {
    return blockProgress.some(progress => progress.blockId === blockId && progress.isCompleted);
  };

  const isAssessmentCompleted = (assessmentId: number): boolean => {
    return assessmentProgress.some(progress => progress.assessmentId === assessmentId && progress.isCompleted);
  };
