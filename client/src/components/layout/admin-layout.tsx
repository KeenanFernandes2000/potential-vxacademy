...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800">
        ...
        <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setSidebarOpen(false)}>
          <div className="absolute top-0 left-0 bottom-0 w-64 bg-slate-900/90 backdrop-blur-xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 flex items-center justify-center border-b border-white/10">
              ...
              <button onClick={() => setSidebarOpen(false)} className="text-white/70 hover:text-white ml-auto">
                <X className="h-5 w-5" />
              </button>
            </div>
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex md:flex-col w-64 bg-slate-900/95 backdrop-blur-xl text-white shadow-2xl border-r border-white/10">
        <div className="p-6 flex items-center justify-center border-b border-white/10">
          <GraduationCap className="h-8 w-8 text-teal-400 mr-3" />
          <span className="font-bold text-xl bg-gradient-to-r from-teal-400 to-cyan-400 bg-clip-text text-transparent">VX Academy</span>
        </div>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Mobile header */}
        <header className="md:hidden bg-white/80 backdrop-blur-xl shadow-sm z-10 border-b border-slate-200/50">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-2">
              <GraduationCap className="h-6 w-6 text-teal-500" />
              <h1 className="text-lg font-bold bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text text-transparent">VX Academy</h1>
            </div>
            <div className="flex items-center gap-2">
              <Button
                asChild
