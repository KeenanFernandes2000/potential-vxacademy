...
export class CertificatePDFService {
   ...
   */
  static async generateCertificate(templatePath: string, certificateData: CertificateData): Promise<Buffer> {
    try {
      ...
      console.log('🔄 Placeholder mappings created:', Object.keys(placeholderMap));
      ...
      try {
        ...
        console.log('ℹ️ No interactive form fields found or error accessing them');
      }
   ...
   */
  private static createPlaceholderMap(data: CertificateData): Record<string, string> {
    return {
   ...
   */
  private static async fillFormFields(form: any, fields: any[], placeholderMap: Record<string, string>) {
    ...
    fields.forEach((field, index) => {
      ...
      if (fieldType === 'PDFTextField') {
        ...
        Object.keys(placeholderMap).forEach(placeholder => {
          const normalizedPlaceholder = placeholder.toLowerCase().replace(/[{}[\]]/g, '');
          if (fieldName.toLowerCase() === normalizedPlaceholder) {
            value = placeholderMap[placeholder];
            console.log(`✅ EXACT MATCH: "${fieldName}" matches "${placeholder}" -> "${value}"`);
          }
        });
        
        // If no exact match, try pattern matching
        if (!value) {
          if (normalizedFieldName.includes('username') || normalizedFieldName.includes('name')) {
            value = placeholderMap['{{USER_NAME}}'];
            console.log(`🎯 PATTERN MATCH (name): "${fieldName}" -> "${value}"`);
          } else if (normalizedFieldName.includes('coursename') || normalizedFieldName.includes('course')) {
            value = placeholderMap['{{COURSE_NAME}}'];
            console.log(`🎯 PATTERN MATCH (course): "${fieldName}" -> "${value}"`);
          } else if (normalizedFieldName.includes('date') || normalizedFieldName.includes('completion')) {
            value = placeholderMap['{{DATE}}'];
            console.log(`🎯 PATTERN MATCH (date): "${fieldName}" -> "${value}"`);
          } else if (normalizedFieldName.includes('certificateid') || normalizedFieldName.includes('id')) {
        ...
        if (value) {
          ...
          try {
            ...
            console.log(`✅ Verification: field '${fieldName}' now contains: "${setText}"`);
          } catch (e) {
            console.log(`⚠️ Could not verify field content: ${e.message}`);
          }
        } else {
   ...
   */
  private static async overlayTextOnPDF(pages: any[], placeholderMap: Record<string, string>) {
    ...
    const helveticaBold = await firstPage.doc.embedFont(StandardFonts.HelveticaBold);
    ...
    textOverlays.forEach((overlay, index) => {
      ...
      if (overlay.text && overlay.text.trim() && overlay.text !== 'undefined') {
        try {
          ...
          const x = overlay.centered 
            ? overlay.x - (textWidth / 2) 
            : overlay.x;
          ...
          overlaysAdded++;
          console.log(`✅ Added ${overlay.label}: "${overlay.text}" at (${x.toFixed(1)}, ${overlay.y}) with ${overlay.backgroundColor ? 'background' : 'no background'}`);
        } catch (drawError) {
    ...
    console.log(`🎨 Text overlay complete: ${overlaysAdded}/${textOverlays.length} overlays added`);
  }
