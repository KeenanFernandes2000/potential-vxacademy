...
export function AiTutorSection() {
  ...
  const messageMutation = useMutation({
    mutationFn: async (message: string) => {
      const res = await apiRequest("POST", "/api/ai-tutor/message", { message });
      return res.json();
    },
    onSuccess: () => {
      setMessage("");
      queryClient.invalidateQueries({ queryKey: ["/api/ai-tutor/conversation"] });
    },
  });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-heading text-lg font-semibold text-neutrals-800">AI Learning Assistant</h2>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/ai-tutor/conversation"] })}
          ...
          <div className="flex items-start mb-3">
            ...
            <div className="flex-1">
              <p className="text-sm text-neutrals-700">
                Hi! I'm your EHC Academy AI assistant. I can help you with questions about your courses, healthcare best practices, or patient care scenarios. How can I assist you today?
              </p>
            </div>
          </div>
        )}
      ...
      <div className="mt-4">
        <p className="text-xs text-neutrals-500">
          Try asking about: <Link href="/ai-tutor"><a className="text-primary">assessment preparation</a></Link>, <Link href="/ai-tutor"><a className="text-primary">difficult concepts</a></Link>, or <Link href="/ai-tutor"><a className="text-primary">patient care scenarios</a></Link>
        </p>
      </div>
    </div>
  );
